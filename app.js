...
let state = JSON.parse(localStorage.getItem("dv3-gem-state") || "{\"gems\":[],\"builds\":[]}");
let selectedSets = [];
let lastResult = null;
const $ = id => document.getElementById(id);
function save(){ localStorage.setItem("dv3-gem-state", JSON.stringify(state)); render(); }
...
function renderInventory(){
  ...
  root.innerHTML = CORES.map(core=>{
    ...
    return `<div class="group"><div class="group-title"><span>${core}</span><span>${gems.length}</span></div>${bySet}</div>`;
  }).join("");
}
...
window.renameBuild=(id,name)=>{ const b=state.builds.find(x=>x.id===id); if(b){b.name=name; localStorage.setItem("dv3-gem-state", JSON.stringify(state));} };
